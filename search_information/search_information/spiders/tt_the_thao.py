...
class TtTheThaoSpider(scrapy.Spider):
    name = 'tt_the_thao'
    allowed_domains = ['tuoitre.vn/the-thao']
    start_urls = []

    def start_requests(self):
        for i in range(1, 201):
            page = 'http://tuoitre.vn/the-thao/trang-'+str(i)+'.htm'
    ...
    def parse(self, response):
        for item_url in response.css('#content > div > div.list-middle > div.list-middle-block > div.w490.fl.stream-home.list-middle-content > div > ul > li > a ::attr(href)').extract():
